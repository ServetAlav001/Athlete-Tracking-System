...
class SporcuTakipGUI extends JFrame {
    ...
    public SporcuTakipGUI() {
        ...
        // **Önceki sürümden kaldırıldı: sureBitisKontrol() çağrısı burada yok.**

        cardLayout.show(contentPanel, "KAYIT");
    }

    // YENİ EKLENEN METOT: Süresi Biten/Yaklaşan Sporcuları Kontrol Eder ve Bildirim Gösterir
    private void sureBitisKontrol() {
        List<Sporcular> sporcular = yonetim.getDbYonetimi().tumSporculariGetir();
        LocalDateTime bugun = LocalDateTime.now();
        StringBuilder yakinBitisMesaji = new StringBuilder();
        StringBuilder sureDolduMesaji = new StringBuilder();

        boolean yakinBitisVar = false;
        boolean sureDolduVar = false;
        ...
        for (Sporcular s : sporcular) {
            ...
            // 7 gün veya daha az kalanları (ve süresi dolmamış olanları) yakala
            if (farkGun > 0 && farkGun <= 7) {
                yakinBitisMesaji.append(String.format("• ID %d: %s %s | Kalan Gün: %d\n",
                        s.getId(), s.getIsim(), s.getSoyad(), farkGun));
                yakinBitisVar = true;
            } else if (farkGun <= 0) {
                // Süresi dolanları listeye ekle
                sureDolduMesaji.append(String.format("• ID %d: %s %s | SÜRESİ DOLDU! (%d gün önce)\n",
                        s.getId(), s.getIsim(), s.getSoyad(), Math.abs(farkGun)));
                sureDolduVar = true;
            }
        }

        // 1. Süresi dolanlar için en önemli uyarı
        if (sureDolduVar) {
            String baslik = "❌ KRİTİK UYARI: Süresi Dolan Sporcular";
            String mesaj = "Aşağıdaki sporcuların kayıt süresi **dolmuştur** ve yenilenmesi gerekmektedir:\n\n" + sureDolduMesaji.toString();
            JOptionPane.showMessageDialog(this, mesaj, baslik, JOptionPane.ERROR_MESSAGE);
        }

        // 2. Süresi yaklaşanlar için uyarı
        if (yakinBitisVar) {
            String baslik = "⚠️ ÖNEMLİ BİLDİRİM: Kayıt Süresi Yakın Olan Sporcular (7 Gün)";
            String mesaj = "Aşağıdaki sporcuların kayıt süresi **7 gün içinde** dolacaktır:\n\n" + yakinBitisMesaji.toString();
            JOptionPane.showMessageDialog(this, mesaj, baslik, JOptionPane.WARNING_MESSAGE);
        }

        // 3. Herhangi bir sorun yoksa bilgi mesajı
        if (!yakinBitisVar && !sureDolduVar) {
            JOptionPane.showMessageDialog(this, "Tüm sporcuların kayıt süresi aktif durumdadır.", "Durum Kontrolü", JOptionPane.INFORMATION_MESSAGE);
        }
    }
    ...
    private JPanel createSidebar() {
        ...
        panel.setLayout(new GridLayout(7, 1, 10, 10)); // Satır sayısı 7'ye çıkarıldı
        ...
        // YENİ BİLDİRİM BUTONU
        JButton btnKontrol = new JButton("🔔 SÜRE KONTROLÜ");
        btnKontrol.setForeground(new Color(0, 0, 0));
        btnKontrol.setBackground(new Color(255, 255, 102)); // Sarı renk tonu
        btnKontrol.setFocusPainted(false);
        btnKontrol.addActionListener(e -> sureBitisKontrol()); // Metodu çağırır
        ...
        panel.add(btnFinans);
        panel.add(btnKontrol); // Buton eklendi
